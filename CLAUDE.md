...
- **Sortino/Sharpe** are NOT reliable from the public `portfolio` equity curve (coarse ~weekly + deposit-contaminated). Use **Hydromancer daily equity** for MaxDD/Calmar/equity-curve, and **fills `realized_pnl` summed per day** for clean (deposit-free) Sharpe/Sortino.
